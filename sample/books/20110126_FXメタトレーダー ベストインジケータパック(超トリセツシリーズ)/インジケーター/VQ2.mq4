...
string short_name = "VQ" + TimeFrameStr + " |  " + Length + " , " + 
...
if(timeprev1<iTime(NULL,TimeFrame,0)) {TurnedDn = false; TurnedUp = 
false; timeprev1=iTime(NULL,TimeFrame,0);}
...
if (CountBars>iBars(NULL,TimeFrame) || CountBars>Bars-Length-1) 
CountBars=MathMin(Bars-Length-1,iBars(NULL,TimeFrame)-Length-1);
if (Crash && CountBars>0){CountBars=CountBars-10; 
...
MC1=iMA(NULL,TimeFrame,Length,0,Method,PRICE_MEDIAN,i+Smoothing);}
   VQ[i] = MathAbs(((MC - MC1) / MathMax(MH - ML,MathMax(MH - MC1,MC1 - 
ML)) + (MC - MO) / (MH - ML)) * 0.5) * ((MC - MC1 + (MC - MO)) * 0.5);
   SumVQ[i] = SumVQ[i + 1] + VQ[i];
   if (Filter > 0) if (MathAbs(SumVQ[i] - SumVQ[i + 1]) < Filter * 
Point) SumVQ[i] = SumVQ[i + 1];
   ...
   if (TimeFrame>Period())
   {
     ...
     for(i=0, int y=0;i<limit;i++) {if (Time[i]<TimeArray1[y]) y++;  
SumVQ[i]=SumVQ_MTF[y];}
   }
...
for (i = CountBars; i >= 0; i--)
  {
   ...
   if (Alerts == true)
      ...
      if ((DIR[i] == 1) && (DIR[i + 1] == -1)) UpArrow[i] = Low[i] - Range*0.5;
      if ((DIR[i] == -1) && (DIR[i + 1] == 1)) DnArrow[i] = High[i] + Range*0.5;
      }
   }
...
if (Alerts == true)
{
   ...
   if (UpArrow[0+p]!=EMPTY_VALUE && TurnedUp==false)
      {
        AlertTXT="VQ BUY:  "+Symbol()+" - "+Period()+"  at  "+ 
DoubleToStr(Close[0],Digits)+ "  -  "+ TimeToStr(CurTime(),TIME_SECONDS);
...
DoubleToStr(Time[0],0),OBJPROP_ARROWCODE,5);
          ObjectSet("BUY SIGNAL: " + 
DoubleToStr(Time[0],0),OBJPROP_COLOR,SignalPriceBUY);
        }
        TurnedDn = false; TurnedUp = true;
      }
   if (DnArrow[0+p]!=EMPTY_VALUE && TurnedDn==false)
      {
        AlertTXT="VQ SELL:  "+Symbol()+" - "+Period()+"  at  "+ 
DoubleToStr(Close[0],Digits)+ "  -  "+ TimeToStr(CurTime(),TIME_SECONDS);
        Alert(AlertTXT); if (EmailON) SendMail(AlertTXT,AlertTXT);
        if (SignalPrice == true)
        {
...
DoubleToStr(Time[0],0),OBJPROP_ARROWCODE,5);
          ObjectSet("SELL SIGNAL: " + 
DoubleToStr(Time[0],0),OBJPROP_COLOR,SignalPriceSELL);
        }
